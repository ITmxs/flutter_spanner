...
import 'package:spanners/cTools/yin_flutter_chart/flutter_chart_bean.dart';
import 'package:spanners/cTools/yin_flutter_chart/flutter_line_painter.dart';
...
class ChartLinePainter extends BasePainter {
  ...
  void initPath(Size size) {
    if (path == null) {
      if (chartBeans != null && chartBeans.length > 0 && maxMin[0] > 0) {
        ...
        int length = chartBeans.length > 31 ? 31 : chartBeans.length;
  ...
  void drawRuler(Canvas canvas, Paint paint) {
    if (chartBeans != null && chartBeans.length > 0) {
      int length = chartBeans.length > 31 ? 31 : chartBeans.length; //最多绘制31个
      ...
      for (int i = 0; i < length; i++) {
        ...
        TextPainter(
          ...
          ..paint(
              canvas,
              Offset(startX + dw * i - 20,
                  startY + 7)); // startY+ basePadding x轴坐标据x轴距离
      ...
      for (int i = 0; i < yLength; i++) {
        if (isShowYValue) {
          ...
          TextPainter(
              ...
              text: TextSpan(
                  text:
                      '¥${(dValue * i) > 10000 ? (dValue * i) / 10000 : (dValue * i)}${(dValue * i) > 10000 ? 'W' : ''}',
